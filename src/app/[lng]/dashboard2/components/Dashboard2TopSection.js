...
import Dashboard2MonthNavigator from "./Dashboard2MonthNavigator";
import { useFormat } from "@/hooks/useFormat";

const Dashboard2TopSection = ({
  t,
  currentMonth,
  currentYear,
  getPreviousMonth,
  ...
  const { biggestIncome = 0, biggestIncomeDate, monthExpenses = 0, monthIncome = 0 } =
    useDashboardContext();
  ...
  useEffect(() => {
    ...
    if (
      (lastRecurrenceDate && moment().isAfter(parseInt(lastRecurrenceDate, 10), "day")) ||
      !lastRecurrenceDate
    ) {
...
export default Dashboard2TopSection;
