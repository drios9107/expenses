		...
		() => [
			{
				...
				renderCell: ({ row }) => (
					<Tooltip title={row?.description}>
						<Typography variant="body1" color={getLineColor(row)}>
							{row?.category?.name}
						</Typography>
					</Tooltip>
				),
			...
			},
			{
				...
				renderCell: ({ row }) => (
					<TypographyIconCell sx={{ width: '100%', textAlign: 'center' }} color={getLineColor(row)}>
						{row?.isExpense ? <Check sx={styles.icon} /> : <DoNotDisturb sx={styles.icon} />}
					</TypographyIconCell>
				)
			},
			{
				...
				renderCell: ({ row }) => (
					<TypographyIconCell sx={{ width: '100%', textAlign: 'center' }} color={getLineColor(row)}>
						{row?.isRecurrent ? <Check sx={styles.icon} /> : <DoNotDisturb sx={styles.icon} />}
					</TypographyIconCell>
				)
			},
			{
