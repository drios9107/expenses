...
const RawMultiTransactions = ({ onClose = () => {} }) => {
	...
	const updateLine = useCallback(() => {
		...
		setAllData((prev = []) => {
			...
			const result = [...prev]
			if (index > -1) result[index] = { id, description }
	...
	useEffect(() => {
		if (delay?.description && delay?.description !== '') updateLine(delay)
	}, [delay, updateLine])
				...
				<Box sx={styles.container}>
					{allData.map((i, index) => (
						<MuiTextfieldWithoutControl
							...
							options={{ label: `${t('transactions:description')} ${index + 1}`, multiline: true }}
						/>
					))}
				</Box>
